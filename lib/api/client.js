...
const getApiBase = () => {
  ...
  const base = process.env.KABURLU_BACKEND_URL || process.env.NEXT_PUBLIC_BACKEND_URL || process.env.NEXT_PUBLIC_API_BASE || 'https://app.kaburlumedia.com/api/v1'
...
export const apiClient = {
  ...
  getAuthHeaders() {
    ...
    // Best practice: when using our server-side proxy, do NOT send JWT from the browser.
    if (typeof window !== 'undefined' && baseUrl.startsWith('/api/')) {
      return {
