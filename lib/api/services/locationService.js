...
export const locationService = {
  ...
  async searchCombined(query, { tenantId, limit = 20 } = {}) {
    ...
    const raw = await apiClient.get('/locations/search-combined', params)
  ...
  /** State / district / mandal + IDs for union member forms after search-combined pick. */
  fieldsFromPick(item) {
    if (!item) {
      return {
        state: '',
    ...
    return {
      ...
      districtId,
      mandal,
      workingArea: this.formatItemLabel(item),
    }
  },
