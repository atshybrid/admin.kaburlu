...
export async function getEpaperSettings(tenantId, domainId) {
  return await request(`/api/v1/tenants/${tenantId}/domains/${domainId}/settings`)
}
...
export async function replaceEpaperSettings(tenantId, domainId, settings, autoSeo = true) {
  ...
  const path = `/api/v1/tenants/${tenantId}/domains/${domainId}/settings${params.toString() ? '?' + params.toString() : ''}`
...
export async function autoGenerateSeo(tenantId, domainId) {
  return await request(`/api/v1/tenants/${tenantId}/domains/${domainId}/settings/seo/auto`, {
    method: 'POST'
  })
}
