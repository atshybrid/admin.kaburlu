...
import { unwrapSyndicationResponse } from '../../syndication/apiResponse'
...
async function call(method, path, body) {
  try {
    let response
    if (method === 'GET') response = await apiClient.get(path)
    else if (method === 'POST') response = await apiClient.post(path, body)
    else if (method === 'PATCH') response = await apiClient.patch(path, body)
    else if (method === 'DELETE') response = await apiClient.delete(path)
    else throw new Error(`Unsupported method: ${method}`)
    return unwrapSyndicationResponse(response)
  } catch (error) {
    wrapError(error)
  }
}
