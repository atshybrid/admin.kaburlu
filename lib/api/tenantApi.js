...
async function request(path, options = {}) {
  ...
  // Remove /api/v1 prefix if present since proxy adds it
  ...
  // 🔍 DEBUG: Log response
  if (typeof window !== 'undefined') {
    console.log(`📬 Response: ${res.status} ${res.statusText} for ${cleanPath}`)
  }
...
export const seoApi = {
  ...
  get: (tenantId) => request(`/api/v1/tenant-theme/${tenantId}/seo`),
  
  /** PATCH /tenant-theme/:tenantId/seo */
  patch: (tenantId, payload) => request(`/api/v1/tenant-theme/${tenantId}/seo`, {
    method: 'PATCH',
    body: JSON.stringify(payload),
  }),
}
...
export const pagesApi = {
  ...
  list: (tenantId) => request(`/api/v1/tenants/${tenantId}/pages`),
  
  /** GET /tenants/:tenantId/pages/:slug */
  get: (tenantId, slug) => request(`/api/v1/tenants/${tenantId}/pages/${slug}`),
  
  /** PUT /tenants/:tenantId/pages/:slug (create or update) */
  upsert: (tenantId, slug, payload) => request(`/api/v1/tenants/${tenantId}/pages/${slug}`, {
    method: 'PUT',
    body: JSON.stringify(payload),
  }),
  
  /** PATCH /tenants/:tenantId/pages/:slug */
  patch: (tenantId, slug, payload) => request(`/api/v1/tenants/${tenantId}/pages/${slug}`, {
    method: 'PATCH',
    body: JSON.stringify(payload),
  }),
  
  /** DELETE /tenants/:tenantId/pages/:slug */
  delete: (tenantId, slug) => request(`/api/v1/tenants/${tenantId}/pages/${slug}`, {
    method: 'DELETE',
  }),
}
...
export const reportersApi = {
  ...
  list: (tenantId) => request(`/api/v1/tenants/${tenantId}/reporters`),
  
  /** GET /tenants/:tenantId/reporters/:reporterId */
  get: (tenantId, reporterId) => request(`/api/v1/tenants/${tenantId}/reporters/${reporterId}`),
  
  /** POST /tenants/:tenantId/reporters */
  create: (tenantId, payload) => request(`/api/v1/tenants/${tenantId}/reporters`, {
    method: 'POST',
    body: JSON.stringify(payload),
  }),
  
  /** PUT /tenants/:tenantId/reporters/:reporterId */
  update: (tenantId, reporterId, payload) => request(`/api/v1/tenants/${tenantId}/reporters/${reporterId}`, {
    method: 'PUT',
    body: JSON.stringify(payload),
  }),
  ...
  setAutoPublish: (tenantId, reporterId, autoPublish) => 
    request(`/api/v1/tenants/${tenantId}/reporters/${reporterId}/auto-publish`, {
      method: 'PATCH',
...
export const tenantCategoriesApi = {
  ...
  update: (tenantId, categorySlugs) => request(`/api/v1/tenants/${tenantId}/categories`, {
    method: 'PUT',
