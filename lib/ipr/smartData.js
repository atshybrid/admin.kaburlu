...
export function reporterToStaffMember(reporter = {}) {
  const address = joinAddress(
    reporter.address,
    reporter.district?.name || reporter.districtName,
    reporter.state?.name || reporter.stateName,
    reporter.mandal?.name || reporter.mandalName,
    reporter.level ? `${reporter.level} level` : '',
  )
  return {
    role: 'Reporter',
    name: reporter.fullName || reporter.name || '',
    phone: reporter.mobileNumber || reporter.phone || '',
    address,
    salary: reporter.salary ? String(reporter.salary) : '',
  }
}

export function applyReportersToStaff(staff = [], reporters = []) {
  const list = (Array.isArray(reporters) ? reporters : []).filter((reporter) => reporter?.fullName || reporter?.name || reporter?.mobileNumber)
  if (!list.length) return staff

  const active = list.filter((reporter) => reporter.active !== false)
  const reporterRows = (active.length ? active : list).map(reporterToStaffMember)
  const nonReporterStaff = staff.filter((member) => member.role !== 'Reporter')
  const reporterSlot = staff.find((member) => member.role === 'Reporter') || { role: 'Reporter', name: '', phone: '', address: '', salary: '' }

  return [
    ...nonReporterStaff,
    { ...reporterSlot, ...reporterRows[0], role: 'Reporter' },
    ...reporterRows.slice(1),
  ]
}

export function applyAutoStaff(form, reporters) {
  const defaults = createSmartDefaults().staff.map((member) => {
  ...
  return reporters?.length ? applyReportersToStaff(defaults, reporters) : defaults
}
