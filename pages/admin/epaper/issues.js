...
function EPaperIssuesContent() {
  ...
  useEffect(() => {
    ...
    fetchTextOrRedirect(`/api/admin/epaper/publication-editions?${params.toString()}`)
      .then((t) => {
        const j = JSON.parse(t)
        const items = j?.items || j?.data?.items || j?.data || []
        setEditions(Array.isArray(items) ? items : [])
        if (Array.isArray(items) && items[0]?.id) setEditionId(items[0].id)
      })
  ...
  async function findIssue(e) {
    ...
    try {
      ...
      if (targetKind === 'subEdition') params.set('subEditionId', subEditionId)
      else params.set('editionId', editionId)
      if (canOverrideTenant && tenantId) params.set('tenantId', tenantId)
  ...
  return (
    <div className="p-6">
        <div className="mb-6">
          <h1 className="text-xl font-bold text-slate-900">ePaper (PDF) — Find Issue</h1>
          <p className="text-sm text-slate-500">Lookup an issue by date + edition/sub-edition.</p>
        </div>
        ...
        <form onSubmit={findIssue} className="bg-white rounded-xl border p-5 space-y-4">
          {canOverrideTenant && (
            <div>
          ...
          <div className="mt-6 bg-white rounded-xl border p-5">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <div className="text-sm text-slate-700">ID: <span className="font-mono">{data.issue.id}</span></div>
                <div className="text-sm text-slate-700">Page count: {data.issue.pageCount ?? '—'}</div>
                {data.issue.pdfUrl && (
                  <div className="text-sm text-slate-700">PDF: <a className="text-brand underline" href={data.issue.pdfUrl} target="_blank" rel="noreferrer">open</a></div>
                )}
              </div>
