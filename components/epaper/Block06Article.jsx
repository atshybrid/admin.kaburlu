...
import styles from './Block06Article.module.css'
import { analyzeBlock08Visuals } from '../../lib/epaper/block08VisualAnalysis'
import { buildBlock06Composition } from '../../lib/epaper/block06Composition'
import { buildBlock06ColumnModels } from '../../lib/epaper/block06ColumnModel'
import { mergeEditorialParagraphs } from '../../lib/epaper/block08ParagraphMerge'
import { assignBlock06ColumnText } from '../../lib/epaper/block06ThreadFlow'
...
import {
  alignColumnTextsToRenderedBottoms06,
  measureRenderedColumnTextBottoms06,
} from '../../lib/epaper/block06CrossColumnFlow'
...
import {
  ...
  BLOCK_06A_IMAGE,
  block06ImageObstaclePx,
} from '../../lib/epaper/wideBlockRules'
import {
  block06ImageMaxHeightPx,
  computeBlock06ImageFrameHeight,
} from '../../lib/epaper/block06ImageFrame'
...
/**
 * BLOCK-06A — 2-column CSS grid (same engine as 08A).
 * Col1: headlines (top) → text | Col2: image (top) → text
 * Continuous text col1→col2 (below image); even bottoms; H&J; hyphen at column breaks.
 */
export default function Block06Article({
  title,
  subtitle = '',
  category = 'general',
  dateline = '',
  ...
  const composition = useMemo(
    () =>
      buildBlock06Composition(visuals, leadImages, headlinePoints.length > 0, {
        title,
        category,
      }),
  ...
  const colorOpts = useMemo(
    () => ({
      ...
      hasSubtitle,
      wideBlockTitle: true,
      wideTitleMinPx: typography.titleMinPx ?? BLOCK_06A_TITLE.minPx,
      wideTitleMaxPx: typography.titleMaxPx ?? BLOCK_06A_TITLE.maxPx,
      preferMultiLineOnly:
        typography.forceMultiLine || measuredTitleInkRatio >= 0.85,
      wideTitleMaxLines: BLOCK_06A_TITLE.maxLines,
    }),
    [
      titleColor,
      titleColorEnabled,
  ...
  const colWidthPx = useMemo(
    () => wideColumnContentWidthPx(BLOCK_06A_DIMENSIONS.nativeWidthPx, COLUMN_COUNT, 2, 2),
    []
  )
    ...
    return columnModels.map((model, colIdx) => {
      ...
      if (model.images?.length) {
        model.images.forEach((slot, imgIdx) => {
          ...
          h += block06ImageObstaclePx(slot.role || 'primary', !!slot.image?.caption, est)
        })
      }
      return h
    })
  ...
  const schedulePostPartitionAlign = useCallback(
    (texts) => {
      ...
      alignTimerRef.current = setTimeout(() => {
        ...
        if (!grid || grid.children?.length !== COLUMN_COUNT) return
        if (alignPassRef.current >= 8) {
          markLayoutSettled()
          return
        }

        const bottoms = measureRenderedColumnTextBottoms06(grid)
        if (!bottoms) {
        ...
        if (spread <= 5 && centerOk) {
          markLayoutSettled()
          return
        }

        setColumnTexts((prev) => {
          const aligned = alignColumnTextsToRenderedBottoms06(prev, grid)
    ...
    [columnModels, markLayoutSettled]
  )
  ...
  const runColumnPartitionNow = useCallback(async () => {
    ...
    const obstacleTotals = domObstacles

    const colWidths = uniformBlock08ColumnWidths(
      columnsRef.current,
      COLUMN_COUNT,
      BLOCK_06A_DIMENSIONS.nativeWidthPx
    )

    const partitionKey = `${bodyFlowKey}|${obstacleTotals.join(',')}|${colWidths.join(',')}`
    if (partitionKey === lastPartitionKeyRef.current) return
    lastPartitionKeyRef.current = partitionKey

    const cols = assignBlock06ColumnText(
      bodyItems,
      columnModels,
      obstacleTotals,
      null,
      colWidths,
      { dateline, showDateline }
    )
  ...
  useLayoutEffect(() => {
    ...
    const run = async () => {
      ...
      const sizes = fitTitleLinesToRail(
        ...
        initial,
        typography.titleMinPx ?? BLOCK_06A_TITLE.minPx,
  ...
  useLayoutEffect(() => {
    let cancelled = false
    const schedule = () => {
      if (!cancelled && !layoutFrozenRef.current) runColumnPartition()
    }
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const textBottoms = measureRenderedColumnTextBottoms06(columnsRef.current) || []
  ...
  const familyClass =
    styles[`family${composition.layoutFamily.replace(/08A-|06A-/g, '')}`] || ''
  ...
  return (
    <div
      ref={blockRef}
      className={`${styles.block06a} ${familyClass}`}
      data-block-code="BLOCK-06A"
      data-engine-version="threaded-v3.4"
      data-locked="true"
      data-layout-family={composition.layoutFamily}
      data-flow-model="threaded-2col"
      lang="te"
    >
      ...
      >
        {!layoutReady && bodyItems.length > 0 ? (
          <BlockColumnLayoutSkeleton columns={2} imageColumnIndex={1} columnGap={16} />
        ) : null}
              ...
              {model.images.map((slot, imgIdx) => (
                ...
                >
                  <EditorialCropImage
                    ...
                    role={slot.role || 'primary'}
                    maxImageHeightPx={block06ImageMaxHeightPx(slot.role || 'primary')}
                    widthPct={BLOCK_06A_IMAGE.widthPct}
