...
export default function SmartArticleDrawer({ onClose, onCreated }) {
  ...
  const loadTenants = async () => {
    try {
      ...
      const list = await apiClient.get('/tenants', { full: true })
  ...
  const handleTenantSelect = async (tenantId) => {
    ...
    try {
      ...
      const result = await apiClient.get('/categories', { tenantId })
  ...
  const handleAIProcess = async () => {
    ...
    try {
      ...
      const result = await apiClient.post('/articles/unified', payload)
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <div className="relative mx-auto mt-10 w-full max-w-2xl bg-white rounded-xl shadow-xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <div className="font-semibold">Create Article (AI)</div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {step === 1 && (
            <div className="text-center py-8">
              <h2 className="text-xl font-bold mb-2">Select Tenant</h2>
              <p className="text-sm text-gray-500 mb-6">Choose news organization</p>
              {loadingTenants ? (
                <Loader size={48} />
              ) : (
                <div className="space-y-3">
                  {tenants.map(t => (
                    <button
                      ...
                      className="w-full p-4 rounded-lg border-2 hover:border-brand"
                    >
                      <div className="font-semibold">{t.nativeName || t.name}</div>
                    </button>
                  ))}
                </div>
              )}
          ...
          {step === 2 && (
            <div className="space-y-6">
              <div className="bg-blue-50 border p-4 rounded-lg">
                <div className="font-semibold">{publisherName}</div>
                <div className="text-xs">{categories.length} categories</div>
              </div>
              <textarea
                rows={16}
                className="w-full border-2 rounded-lg p-4"
                placeholder="Enter raw news..."
                value={rawText}
              ...
              <button
                ...
                className="w-full py-4 rounded-lg bg-gradient-to-r from-brand to-brand-dark text-white font-semibold"
              >
                🚀 Generate Article
              </button>
            </div>
